...
/**
 * Created by pinwheel5 on 8/7/2017.
 */
...
public class ChoosePhoto {
    ...
    public Uri handleCameraResult(Uri cameraPictureUrl) {
        try {
            cropPictureUrl = Uri.fromFile(FileUtil.getInstance(mContext).createImageTempFile(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES)));
            return cropPictureUrl;
    ...
    public Intent cropImage(final Uri sourceImage, Uri destinationImage) {
        ...
        if (size == 0) {
            ...
            //((Activity) mContext).startActivityForResult(intent, PIC_CROP);
            return intent;
        } else {
            ...
            if (size == 1) {
                ...
                return i;
               // i.putExtra(AppConstant.CROP_URI,selectedImageUri);
               /* ((Activity) mContext).startActivityForResult(intent, PIC_CROP);*/
            } else {
                Intent i = new Intent(intent);
